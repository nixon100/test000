...
import referenceFlipkart from "../../reference_value/referencevalueF.json";
...
const Main = () => {
  ...
  useEffect(() => {
    document.body.classList.add("Flipkart");
    return () => {
  ...
  return (
    <div class="background-wrapper">
      <img
        ...
        className="background-imageF"
      ></img>
      <button onClick={logoutHandler} className="btn btn-primary text-left1">
        <span>&#10094;</span>
      </button>
      <div className="app2">
        <input
            ...
            <thead>
              <tr>
                {Object.keys(data2[0]).map((key, index) => (
                  <th key={index} className="thF">{key}</th>
                ))}
              </tr>
            </thead>
            <tbody>
